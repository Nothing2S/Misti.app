...
function searchTable(query) {
    ...
    bodyRows.forEach(row => {
        ...
        tds.forEach((cell, idx) => {
            ...
            const isMapCell = cell.classList.contains('map-cell') || cell.classList.contains('details-cell') || cell.querySelector('a.map-link');
            if (!isMapCell) {
                cell.innerHTML = cell.textContent; // Reset any HTML modifications for normal cells
            }
        });
    ...
    bodyRows.forEach((row) => {
        ...
        // Skip the first cell (row number); exclude map/details cells from matching/highlighting
        const cells = Array.from(tds)
            .slice(1)
            .filter(cell => !cell.classList.contains('map-cell') && !cell.classList.contains('details-cell'));
        let hasMatch = false;
...
// Function to highlight matching text within a cell
...
function ensureMapColumn() {
    ...
    let mapIdx = -1;
    if (headerRow) {
        const heads = Array.from(headerRow.cells);
        mapIdx = heads.findIndex(th => th.classList.contains('map-col'));
    }
    rows.forEach((row) => {
        const tds = row.querySelectorAll('td');
        if (!tds.length) return;
        if (mapIdx >= 0 && mapIdx < tds.length) {
            tds[mapIdx].classList.add('map-cell');
        } else {
            const a = row.querySelector('a[href^="http"]');
            if (a) a.closest('td')?.classList.add('map-cell');
        }
    });
}

// Ensure Details column and button per row
function ensureDetailsColumn() {
    const table = document.querySelector('.result-box table');
    if (!table) return;
    const theadRow = table.tHead && table.tHead.rows && table.tHead.rows[0];
    if (theadRow) {
        const already = Array.from(theadRow.cells).some(c => c.classList.contains('details-col'));
        if (!already) {
            const th = document.createElement('th');
            th.textContent = 'លម្អិត';
            th.className = 'details-col';
    ...
    const rows = table.querySelectorAll('tbody tr');
    rows.forEach((row) => {
        const hasDetails = row.querySelector('td.details-cell');
        ...
        btn.textContent = 'ព័ត៌មានលម្អិត';
        btn.addEventListener('click', () => openRowDetails(row));
        td.appendChild(btn);
        row.appendChild(td);
    });
}
...
function extractRowData(row) {
    ...
    for (let i = 1; i < tds.length; i++) {
        const cell = tds[i];
        if (cell.classList.contains('map-cell') || cell.classList.contains('details-cell')) continue;
        const label = headers[i] || `Column ${i}`;
        const link = cell.querySelector('a');
        const value = (cell.textContent || '').trim();
        data.push({ label, value, href: link ? link.getAttribute('href') : '' });
    }
    const mapLink = row.querySelector('td.map-cell a')?.getAttribute('href') || '';
    const rownum = (tds[0] && tds[0].textContent.trim()) || '';
    return { rownum, headers, cells: data, mapLink };
}

function openRowDetails(row) {
    const payload = extractRowData(row);
    try { localStorage.setItem('misti_detail_row', JSON.stringify(payload)); } catch {}
...
function clearSearchHighlighting() {
    ...
    bodyRows.forEach(row => {
        ...
        cells.forEach(cell => {
            cell.classList.remove('highlight', 'search-match');
            // Restore original text content without HTML tags, but skip map cell to preserve link
            const isMapCell = cell.classList.contains('map-cell') || cell.classList.contains('details-cell') || cell.querySelector('a.map-link');
            if (!isMapCell && cell.innerHTML.includes('<mark')) {
...
function refreshKeywordsFromTable() {
    ...
    for (const r of rows) {
        // Skip rownum (first); exclude map/details when building keywords
        const allTds = Array.from(r.querySelectorAll('td'));
        const cells = allTds
            .slice(1)
            .filter(td => !td.classList.contains('map-cell') && !td.classList.contains('details-cell'));
        cells.forEach(cell => {
...
function serializeTableRows() {
    ...
    return rows.map((r) => {
        ...
        const mapTd = r.querySelector('td.map-cell');
        let mapVal = '';
        if (mapTd) {
            const a = mapTd.querySelector('a');
            mapVal = a ? a.getAttribute('href') : (mapTd.textContent || '');
        }
        // Collect first five non-rownum, non-map, non-details text cells to preserve structure
        const values = [];
        for (let i = 1; i < tds.length && values.length < 5; i++) {
            const td = tds[i];
            if (td.classList.contains('map-cell') || td.classList.contains('details-cell')) continue;
            values.push(td.textContent || '');
        }
        while (values.length < 5) values.push('');
        return [...values, mapVal];
    });
}
...
function appendRow(cols) {
    ...
    btn.className = 'details-btn';
    btn.textContent = 'ព័ត៌មានលម្អិត';
    btn.addEventListener('click', () => openRowDetails(tr));
...
(function initSortableTable(){
    ...
    ths.forEach((th, idx) => {
        ...
        const isDetails = th.classList.contains('details-col');
        th.setAttribute('role', 'columnheader');
        th.setAttribute('aria-sort', 'none');
        if (isRownum || isMap || isDetails) {
            // Not sortable
